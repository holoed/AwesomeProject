...
var monadErrorMaybe = new MonadError(function (_283) {
    return function (f) {
        if (_283 instanceof Data_Maybe.Nothing) {
            return f(Prelude.unit);
        };
        if (_283 instanceof Data_Maybe.Just) {
            return new Data_Maybe.Just(_283.value0);
        };
...
var monadErrorEither = new MonadError(function (_282) {
    return function (h) {
        if (_282 instanceof Data_Either.Left) {
            return h(_282.value0);
        };
        if (_282 instanceof Data_Either.Right) {
            return new Data_Either.Right(_282.value0);
        };
        throw new Error("Failed pattern match");
    };
        ...
        return function (act) {
            return function (handler) {
                var handle = function (e) {
                    var _1225 = p(e);
                    if (_1225 instanceof Data_Maybe.Nothing) {
                        return throwError(__dict_MonadError_2)(e);
                    };
                    if (_1225 instanceof Data_Maybe.Just) {
                        return handler(_1225.value0);
                    };
                    throw new Error("Failed pattern match");
                };
